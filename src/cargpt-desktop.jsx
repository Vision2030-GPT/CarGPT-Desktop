...
.detail-hero-img {
  height:320px; border-radius:var(--radius); display:flex;
  align-items:center; justify-content:center;
      ...
      {GARAGE.map(g => (
        <div key={g.id} className="card mb-4">
          <div className="flex gap-4 items-center mb-4">
            <div style={{fontSize:48,width:72,height:72,display:"flex",alignItems:"center",justifyContent:"center",background:"#F3F4F6",borderRadius:12}}>{g.img}</div>
            <div>
    ...
    return (
      <div style={{paddingBottom:80}}>
        ...
        <div className="detail-layout">
          {/* Left column */}
          <div>
            <div className="detail-hero-img mb-4">{v.img}</div>
          ...
          <div className="detail-sidebar">
            ...
            <div className="card">
              ...
              <div className="text-md font-bold">{dl.name}</div>
              <div className="text-xs text-muted mb-2">{dl.location} · ⭐ {dl.rating} ({dl.reviews} reviews)</div>
              <div className="text-xs text-muted">Response: {dl.responseTime} · Trust Score: {dl.trustScore}/100</div>
              <button className="btn btn-primary btn-block btn-sm mt-3" onClick={()=>openDChat(v.id)}>Message Dealer</button>
            </div>
    ...
    switch(activeModal) {
      ...
      case "compare":
        return <SlideOver show={true} onClose={closeModal} title="⚖️ Compare Cars">
          ...
          <div className="flex gap-2 mb-4">{compCars.map((c,i)=><div key={i} style={{flex:1,textAlign:"center"}}><div style={{fontSize:36}}>{c.img}</div><div className="text-xs font-bold">{c.make} {c.model}</div></div>)}</div>
